...
class AnomalyDetectionDataset(data.Dataset):
    def __init__(self, main_path, img_size=224, transform=None, mode="train",  data_ratio=1, ar=0.):
        ...
        if mode == "train":
            ...
            normal_l = data_dict["train"]["unlabeled"]["0"] # 4000
            abnormal_l = data_dict["train"]["unlabeled"]["1"] # 5012
            total_train_num = len(train_normal) + int(4000)
            if ar==0:
                if data_ratio<0.5:
                    train_l =  train_normal[:int(total_train_num*data_ratio)]
                else:
                    train_l = train_normal + normal_l[:int(total_train_num*data_ratio-len(train_normal))]
                self.labels += len(train_l)  * [0]  
            else:
                total_train_num = len(train_normal)  # only use half of the normal images for saving time
                train_l = train_normal + abnormal_l[:int(total_train_num*ar/(1-ar))]
                self.labels += total_train_num * [0] + len(abnormal_l) * [1]
            ...
            print("Loaded {} normal images, "
                  "{}  abnormal images. {:.3f}s".format(len(train_l), int(total_train_num*ar/(1-ar)),
                                                                   time.time() - t0))
        else:  # test
...
class AnomalyDetectionDataset_original(data.Dataset):
    def __init__(self, main_path, img_size=64, transform=None, mode="train", extra_data=0, ar=0.):
        ...
        if mode == "train":
            ...
            # full train
            #train_l = train_normal + normal_l
            # half of 3851
            train_l = train_normal[:int(3851/2)]
