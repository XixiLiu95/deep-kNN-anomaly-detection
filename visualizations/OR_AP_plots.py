...
with plt.style.context(['science', 'ieee']):
    ...
    for index in range(len(outlier_ratio)):
        ax.text(outlier_ratio[index],  AUROC_values[0,index], round(AUROC_values[0, index],3), size=6,verticalalignment='bottom')
        ax.text(outlier_ratio[index],  AUROC_values[1,index], round(AUROC_values[1, index],3), size=6,verticalalignment='top')
    ...
    plt.xlim([-0.005, 0.11])
    plt.show()
    fig.savefig(f'all_plots/{dataset}_OR_AP.pdf', dpi=600)
    plt.close()
    #fig, ax = plt.subplots(nrows=1, ncols=1,  sharex='col', sharey='row')
