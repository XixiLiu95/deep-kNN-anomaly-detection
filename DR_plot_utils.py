...
with plt.style.context(['science', 'ieee']):
    ...
    for index in range(len(data_ratio)):
        ...
        if index ==2:
              ...
              ax.text(data_ratio[index],  AUROC_values[1,index], round(AUROC_values[1,index],3), size=6, horizontalalignment='right' )
        else:
    ...
    #ax.scatter( 0.49, 0.798, marker='P',c = 'salmon', label = 'f-AnoGAN $($DR$=0.49)$',s=1)
    #ax.scatter( 0.49, 0.873, marker='X',c = 'olive', label = 'DDAD-AE-U $($DR$=0.49)$', s=1)
    ax.scatter( 0.5, 0.763, marker='P',c = 'salmon', label = 'f-AnoGAN $($DR$=0.5)$',s=1)
    ax.scatter( 0.5, 0.743, marker='X',c = 'olive', label = 'DDAD-AE-U $($DR$=0.5)$', s=1)
    ...
    plt.xlim([0, 1])
    plt.show()
    fig.savefig(f'all_plots/{aug}_augmentations_{dataset}_DR_AUROC.pdf', dpi=600)
    plt.close()
    #fig, ax = plt.subplots(nrows=1, ncols=1,  sharex='col', sharey='row')
